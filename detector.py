...
def encode_faces(model: str = "hog", encoding_location: Path = DEFAULT_ENCODED_PATH) -> None:
...
def recognise_face(image_location: str, model: str = "hog", encoding_location: Path = DEFAULT_ENCODED_PATH):
    ...
    input_image = face_recognition.load_image_file(Path(f"validation\\{image_location}"))
    input_image_locations = face_recognition.face_locations(input_image, model=model)
    input_image_encodings = face_recognition.face_encodings(input_image, input_image_locations)
    
    pillow_image = Image.fromarray(input_image) # create pillow image to open
    draw = ImageDraw.Draw(pillow_image)  # create draw object to draw rect and text on image
