...
parser = argparse.ArgumentParser(description="Face Recognition Model")  # create parser object

parser.add_argument("--train", dest="tr", action="store_true", help="Train the model")  # train
parser.add_argument("--test", dest="t", action="store_true", 
                    help="Give location with -l along with --test to test the model, else use all images in validation")
parser.add_argument("-l", "--location", action="store", help="Provide image location to test model")
parser.add_argument("-m", "--model", action="store", default="hog", choices=["hog", "cnn"], help="Choose model to use")
